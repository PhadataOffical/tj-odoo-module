...
class Bucket(object):
    ...
    policy = '{"Version":"2012-10-17","Statement":[{"Effect":"Allow","Principal":{"AWS":["*"]},"Action":[' \
             '"s3:GetBucketLocation","s3:ListBucket"],"Resource":["arn:aws:s3:::%s"]},{"Effect":"Allow","Principal":{' \
...
if __name__ == '__main__':
    ...
    # bucket_name = "test"
    service = 'minio.dev.tdaas.phadata.net'
    # service = "192.168.1.101:9000"

    file_name = "addons-tj-back-3-8.zip"
    file_path = "/home/scwang/Projects/odoo-module/addons-tj-3-8.zip"

    # client = Minio(service, access_key='admin', secret_key='12345678', secure=True)
    # client.fput_object(bucket_name, file_name, file_path)
    # url = client.presigned_get_object(bucket_name, file_name, expires=timedelta(days=7))

    minio_obj = Bucket(service=service, access_key="admin", secret_key="12345678", secure=True)
    minio_obj.create_bucket(bucket_name)
    minio_obj.fput_file(bucket_name, file_name, file_path)
    url = minio_obj.presigned_get_file(bucket_name, file_name)
